...
export function loadPair(address: string): Pair {
  let pair = Pair.load(address)
  if (pair === null) {
    throw new Error(`Could not find pair ${address}`)
  }
  return pair
}

export function loadToken(address: string): Token {
...
export function loadBurn(id: string): Burn {
  ...
  if (burn === null) {
    throw new Error(`Could not find Burn ${id}`)
  }
  return burn
}
...
export function loadMint(id: string): Mint {
  ...
  if (mint === null) {
    throw new Error(`Could not find Mint ${id}`)
  }
  return mint
}

export function loadTransaction(id: string): Transaction {
